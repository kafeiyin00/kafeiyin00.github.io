...
```c++
uint8_t stringTest[1024] __attribute__((section(".dma_buffer"))); // 用于发送
...
USART_RECEIVE_TYPE UsartType4 __attribute__((section(".dma_buffer"))) ;// 用于接收
...
if((__HAL_UART_GET_FLAG(&huart4,UART_FLAG_IDLE) != RESET))  // Checks whether the specified UART idle flag is set or not.
  {
      ...
      for (int i = 0; i < rx_len; ++i) {
          HAL_UART_Transmit(&huart4,&UsartType4.RX_pData[i],1,0xffff);
      }
      SCB_CleanInvalidateDCache_by_Addr((uint32_t *)UsartType4.RX_pData,BUFFER_SIZE); //重要
      HAL_UART_Receive_DMA(&huart4, UsartType4.RX_pData, BUFFER_SIZE);  // start DMA interrupt and receives an amount of data in non blocking mode.
  }
```
...
```c++
	if(Uart4IsBusy==0){
        SCB_CleanInvalidateDCache_by_Addr((uint32_t *)stringTest,1024);
